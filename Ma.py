...
def process_buy(item_id, count, item_name, total_price):
    # ✅ اگه آیتم ناشناخته بود، جواب نده
    if item_name not in LIMITS:
        push(f"❌ Unknown: {item_name} - ignored", color=(1, 0.5, 0))
        return
...
def process_bid(item_id):
    ...
    try:
        ...
        # ✅ سریع بزن b sXXX
        CM(f"b {item_id}")
        push(f"⚡ Bid sent: b {item_id}", color=(0, 1, 1))
        gs('dingSmall').play()
    except Exception as e:
        print(f"Bid error: {e}")
    ...
    try:
        ...
        if messages:
            for msg in messages[-3:]:
                ...
                # ✅ اول چک کن پیام b sXXX از دیگران
                # اگه پیام با b شروع بشه و مال خودمون نباشه
                stripped = msg.strip()
                
                # چک کنیم اسم خودمون نباشه
                is_mine = False
                if my_own_name and msg.startswith(my_own_name + ':'):
                    is_mine = True
                
                # اگه مال خودمون نباشه و با b شروع بشه
                if not is_mine and (stripped.startswith('b ') or stripped.startswith('B ')):
                    m2 = BID_PATTERN.match(stripped)
                    if m2:
                        item_id = m2.group(1)
                        # ✅ سریع بزن b sXXX
                        process_bid(item_id)
