...
def get_spam_btn_position(default_x=None, default_y=None):
    try:
        x = ba.app.config.get('spam_btn_x', default_x)
        y = ba.app.config.get('spam_btn_y', default_y)
...
def save_spam_btn_position(x, y):
    try:
        ba.app.config['spam_btn_x'] = float(x)
        ba.app.config['spam_btn_y'] = float(y)
...
def get_spam_messages():
    try:
        saved = ba.app.config.get('spam_msg_list', [])
...
def save_spam_messages(msgs):
    try:
        ba.app.config['spam_msg_list'] = [dict(m) for m in msgs]
        ba.app.config.commit()
    except: pass
...
def get_loop_state():
    try:
        return ba.app.config.get('spam_loop', True)
    except:
        return True
...
def save_loop_state(v):
    try:
        ba.app.config['spam_loop'] = bool(v)
        ba.app.config.commit()
    except: pass
...
def play_sound(name):
    try:
        ba.playsound(ba.getsound(name))
    except:
        pass


def push_msg(text, color=(1, 1, 0)):
    try:
        ba.screenmessage(text, color=color)
    except:
        try:
            _ba.screenmessage(text, color=color)
        except:
            pass


def app_timer(delay, callback):
    try:
        return ba.apptimer(delay, callback)
    except:
        return None
...
def spam_tick():
    ...
    try:
        try:
            conn = bascenev1.get_connection_to_host_info_2()
        ...
        if not in_server:
            spam_active = False
            push_msg("اسپم متوقف شد (از سرور خارج شدی)", color=(1, 0.5, 0))
            return
        ...
        if spam_mode == 'single':
            ...
            spam_timer = app_timer(spam_single_delay, spam_tick)
            ...
            if spam_list_index >= len(spam_list):
                if spam_loop:
                    ...
                    spam_active = False
                    push_msg("لیست تموم شد!", color=(0, 1, 0))
                    return

            spam_timer = app_timer(delay, spam_tick)
...
def start_spam_list():
    ...
    stop_spam()
    if not spam_list:
        push_msg("لیست خالیه!", color=(1, 0.5, 0))
        return
    spam_active = True
    spam_mode = 'list'
    spam_list_index = 0
...
def check_spam_command(msg):
    ...
    try:
        ...
        if content_lower in ('spam off', 'اسپم خاموش', 'توقف اسپم', 'stop spam'):
            stop_spam()
            push_msg("اسپم خاموش شد", color=(1, 0.5, 0))
            return True

        if content_lower in ('spam on', 'اسپم روشن'):
            if spam_single_msg:
                start_spam_single(spam_single_msg, spam_single_delay)
                push_msg("اسپم روشن شد", color=(0, 1, 0))
            else:
                push_msg("هیچ پیامی تنظیم نشده!", color=(1, 0.5, 0))
            return True
        ...
        if match:
            ...
            push_msg(f"اسپم: {message} ({delay} ثانیه)", color=(0, 1, 0))
            return True
...
class SpamEditPlace:
    ...
    def save(s):
        ...
        push_msg(f'ذخیره شد: X={int(s.current_x)} Y={int(s.current_y)}', color=(0, 1, 0))
        play_sound('dingSmallHigh')
...
class AddMsgWindow:
    ...
    def save(s):
        ...
        push_msg(f'اضافه شد: {text}', color=(0, 1, 0))
        play_sound('dingSmallHigh')
...
class SpamWindow:
    ...
    def clear_all(s):
        ...
        spam_list = []
        save_spam_messages(spam_list)
        push_msg('لیست پاک شد!', color=(1, 0.5, 0))
        play_sound('dingSmallLow')
        s.build_grid()
    ...
    def toggle_loop(s):
        ...
        if spam_loop:
            ...
            push_msg('تکرار روشن شد', color=(0, 1, 0))
        else:
            bw(s.loop_btn, label='تکرار: خاموش', color=(0.7, 0.2, 0.2))
            push_msg('تکرار خاموش شد', color=(1, 0.5, 0))
        play_sound('dingSmall')
    ...
    def edit_place(s):
        ...
        try:
            AR.swish(s.w)
            app_timer(0.1, lambda: SpamEditPlace(s.w, s.spam_button))
        except Exception as e:
            print(f"[Spam] edit_place error: {e}")


# ba_meta export plugin
class SpamBot(ba.Plugin):
    def __init__(self):
        ...
        app_timer(1, self.ear)

        app_timer(3.0, lambda: push_msg("Creat By Mahyar", color=(1, 0.5, 0.5)))
        ...
        def e(self, *a, **k):
            ...
            try:
                ...
                bw(b_spam, on_activate_call=lambda: s_open_spam(b_spam))
            except Exception as ex:
                print(f"[Spam] button error: {ex}")

            return r

        def s_open_spam(source):
            try:
                play_sound('swish')
                app_timer(0.1, lambda: SpamWindow(source, source))
            except Exception as e:
